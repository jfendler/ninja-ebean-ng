...
/**
 * All currently supported Ebean properties in file application.conf.
 * 
 * Add them to your application.conf file to alter them.
 * 
 */
public interface NinjaEbeanNGProperties {

	/**
	 * Comma separated list of Ebean models which should be registered at the
	 * EbeanServer.
	 * 
	 * Usually The NinjaCasino plugin registers all models it can get from sub
	 * directory. But sometimes that is not enough (especially when using
	 * external jars.). To make that reliable you can use that property.
	 * 
	 * ebean.models=model.myModel1, model.myModel2
	 * 
	 * To register all classes in a package, simply append a ".*" at the end of
	 * your model name.
	 * 
